...
#define CAM_W 176 // Vi använder QCIF (176x144) i gråskala – 1 byte per pixel
#define CAM_H 144
#define SMALL_W 128
#define SMALL_H 128
...
//uint8_t frame[CAM_W * CAM_H];   // Buffer där hela kamerabilden hamnar (176 * 144 bytes)
...
void downsampleToSmallBilinear(const uint8_t* src, uint8_t* dst) {
    ...
    for (int y = 0; y < SMALL_H; y++) {
        ...
        for (int x = 0; x < SMALL_W; x++) {
            ...
            dst[y * SMALL_W + x] = (uint8_t)(value + 0.5f);
        }
    }
}
...
void trainModelAllImages(){
    for(int i = 0; i < NBR_TRAIN_IMAGES; i++) {
        uint8_t fullImage[2048];
        for (int j = 0; j < 2048; j++) {
            fullImage[j] = pgm_read_byte(&trainImages[i][j]);
        }
        uint8_t label = pgm_read_byte(&trainLabels[i]);
        uint8_t* croppedImage = cropImage(fullImage);
...
void setup() {
    ...
    for (int i = 0; i < EPOCH; i++) {
        ...
        trainModelAllImages();
        Serial.println(calculateAccuracy());
        epoch_count++;
    }
